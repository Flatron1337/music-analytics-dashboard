...
### 🎧 Встроенный аудиоплеер (Audio Preview Player)
- **Мгновенное прослушивание отрывков:** Кнопка Play в каждой карточке трека обращается к Яндекс Музыке и начинает прямое воспроизведение.
- **Floating Mini-Player Bar:** Плавающая неоновая панель плеера над нижней навигацией с отображением обложки, названия, артиста, плавной шкалой прогресса и кнопками паузы.
- **Глобальный синглтон-сервис:** Синхронизация состояния воспроизведения между всеми экранами каталога и дубликатов.

### ⚡ Аудио-профиль медиатеки (Audio Features Profile)
- **Индекс энергии и драйва:** Комплексная оценка взрывной динамики треков (до 81.2% «Максимальный драйв 🔥»).
- **Распределение по BPM:** Классификация по темповым группам (150–180+ BPM, 130–150 BPM, 100–130 BPM, <100 BPM).
- **Профиль воспринимаемой громкости:** Анализ компрессии и плотности мастеринга по стандартам EBU R128 LUFS.

### 🧹 Менеджер дубликатов и ремастеров (Duplicates Finder)
- **Интеллектуальная очистка:** Обнаружение дубликатов треков с учетом вариаций названий (*Remastered*, *Deluxe*, *VIP*, *Edit*).
- **Сводка экономии:** Подсчет количества дублирующихся композиций и суммарного лишнего хронометража.
- **Сравнение и предпрослушивание:** Бейджи «Оригинал» / «Дубликат», прямое превью и переход в Яндекс Музыку.

### 🌐 Экспорт графа связей в автономный HTML
- **Vis.js Network Export:** Генерация self-contained интерактивного файла с неоновой графикой и физикой соавторов для печати, архива и вставки на веб-страницы.
...
- **Аудио-движок:** `audioplayers ^6.8.1`
...
- JDK 21 (например, Android Studio JBR)
...
#### 📦 Быстрая сборка релизного APK для смартфона:
Двойной клик по [build_apk.bat](build_apk.bat) или команда:
```cmd
set JAVA_HOME=E:\Android_studio\jbr
flutter build apk --release
```
Готовый установочный файл появится в `yndex_mobile/app-release.apk`.
